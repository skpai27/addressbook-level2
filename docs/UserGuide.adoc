...
== Finding all persons with the input number: `findNum`

Finds persons whose number matches the given number. +
Format: `find NUMBER`

[NOTE]
====
The search is case sensitive, the order of the keywords does not matter, only the name is searched,
and persons matching the number exactly will be output.
====

Examples:

* `find 1234` +
Returns `John Doe` if his phone number is 1234.
